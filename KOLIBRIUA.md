...
- Розіменування **(`<покажчик>`)** означає, що ви заглядаєте всередину вікна. Ви отримуєте фактичні дані, на які вказує вказівник.
...
2. **Звільнення покажчика від посилання**
    ```c#
    <покажчик>
    ```
...
else Boo(b); // b залишається диктатором
